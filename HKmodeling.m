function [kriging_lf,kriging_hf] = HKmodeling( sample_xh,sample_xl,sample_yh,sample_yl,thetastarter,thetalb,thetaub )
nh = size(sample_xh, 1);
disMat = pdist2( sample_xh, sample_xh);
repetInd = false(nh, 1);
for i = 1 : (nh - 1)
    for j = (i + 1) : nh
        if disMat(i, j) < 1e-5
...
sample_xh(repetInd,:) = [];
sample_yh(repetInd,:) = [];% delete the repetition high-fidelity sample
nl = size(sample_xl, 1);
disMat = pdist2( sample_xl, sample_xl);
repetInd = false(nl, 1);
for i = 1 : (nl - 1)
    for j = (i + 1) : nl
        if disMat(i, j) < 1e-5
...
sample_xl(repetInd,:) = [];
sample_yl(repetInd,:) = [];% delete the repetition low-fidelity sample
kriging_lf=dacefit(sample_xl,sample_yl,'regpoly0','corrgauss',thetastarter,thetalb,thetaub);
kriging_hf=dacefith(sample_xh,sample_yh,'regpoly_c','corrgauss',thetastarter,thetalb,thetaub,kriging_lf);
end


function  [dmodel, perf] = dacefith(S, Y, regr, corr, theta0, lob, upb, dmodel)
...
% Regression matrix
F = feval(regr, S1,dmodel);
F = (F - repmat(mY,m,1)) ./ repmat(sY,m,1);
[mF p] = size(F);
