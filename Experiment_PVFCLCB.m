...
fun_name = 'G4_G4a';
num_q = 5; % number of parallel computers
beta = 0.4; % the coefficient for switch from cons to obj 
...
iter = 0; 
HFE = 0; LFE = 0; NEFE = 0; NEI = 0; % initialize cost records
q_obj = floor(num_q/2); q_con = num_q - q_obj;
...
if sum(index) == 0
    f_min(iter+1)= inf;
    fprintf('iteration: %d, NEI:%.1f, HFE: %d, LFE: %d, NEFE:%.2f, best solution: no feasiable solution.\n', iter, NEI, HFE, LFE, NEFE);
else
    f_min(iter+1)=min(sample_yh(index, :));
    fprintf('iteration: %d, NEI:%.1f, HFE: %d, LFE: %d, NEFE:%.2f, best solution: %f.\n', iter, NEI, HFE, LFE, NEFE, f_min(iter+1));
end
%% Iterative procedure
while NEFE <= hf_budget && f_min(iter+1,1) >  optimum_to_reach 
    ...
    [best_xh,best_xl] = Criterion_Parallel_VFCLCB(q_con,q_obj,krigingobj_hf,krigingobj_lf,krigingcon_hf,krigingcon_lf,w,cr,num_vari,design_space); % infill criterion
    
    % evaluate the candidates with parallel computing structure
    best_yh=[]; best_gh=[]; best_yl=[]; best_gl=[];
    if ~isempty(best_xh)
        best_yh = zeros(size(best_xh, 1), 1); best_gh = zeros(size(best_xh, 1), num_con);
        % -----------------------------------------------------------------
        % ATTENTION: for parallel optimizaiton, you should manage your
        % parallel structure in this part, the code is only provided for
        % reference in mathematical optimizaiton 
        parfor ixh = 1:size(best_xh, 1)
            [best_yh(ixh, :),best_gh(ixh, :)]=feval(fun_name,best_xh(ixh, :),'high');
        end
        % -----------------------------------------------------------------
    ...
    if ~isempty(best_xl)
        best_yl = zeros(size(best_xl, 1), 1); best_gl = zeros(size(best_xl, 1), num_con);
        parfor ixl = 1:size(best_xl, 1)
            [best_yl(ixl, :),best_gl(ixl, :)]=feval(fun_name,best_xl(ixl, :),'low');
        end
    ...
    fprintf('Parallel resorce for con: %d, for obj: %d.\n', q_con, q_obj);
    ...
    [q_con,q_obj] = Calculation_NumPara(q_con,q_obj,best_xh,best_xl,best_gh,best_gl,krigingcon_hf,krigingcon_lf,beta,f_min,iter);
