function [pre_hf,mse_hf,pre_lf,mse_lf] = HKpredictorPara(x,kriging_hf,kriging_lf,Xadd_hf,Xadd_lf)
nx=size(x,1);
if nx>1
...
pre_lf=p_lf;
pre_hf=p_hf;
%% Estimated variance of high-fidelity
if isempty(Xadd_hf)
    mse_hf=s2_hf;
else
    nq_hf=size(Xadd_hf,1);
    theta_hf=kriging_hf.theta;
    IF_hf=zeros(nx,nq_hf);
    for i=1:nq_hf
        sample=Xadd_hf(i,:);
        IF_hf(:,i)=1-exp(-sum((repmat(sample,nx,1)-x).^2.*repmat(theta_hf,nx,1),2));
    end
    mse_hf=s2_hf.*prod(IF_hf,2);
end
%% Estimated variance of low-fidelity
if isempty(Xadd_lf)
    mse_lf=s2_lf;
else
    nq_lf=size(Xadd_lf,1);
    theta_lf=kriging_lf.theta;
    IF_lf=zeros(nx,nq_lf);
    for j=1:nq_lf
        sample=Xadd_lf(j,:);
        IF_lf(:,j)=1-exp(-sum((repmat(sample,nx,1)-x).^2.*repmat(theta_lf,nx,1),2));
    end
    mse_lf=s2_lf.*prod(IF_lf,2);
end
end
